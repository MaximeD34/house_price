...
def convertData(data_raw, withSalePrice):
    ...
    #we only keep CollgCr, Edwards, NAmes, OldTown, the others are grouped in "other"
    Neighborhood[~np.isin(Neighborhood, ["CollgCr", "Edwards", "NAmes", "OldTown"])] = "other"
    uniqueNeighborhood = np.array(["other", "CollgCr", "Edwards", "NAmes", "OldTown"])
    ...
    #we only keep HdBoard, MetalSd, Plywood, VinylSd, Wd Sdng and others are grouped in "other"
    Exterior1st[~np.isin(Exterior1st,  ["HdBoard", "MetalSd", "Plywood", "VinylSd", "Wd Sdng"])] = "other"
    uniqueExterior = np.array(["HdBoard", "MetalSd", "Plywood", "VinylSd", "Wd Sdng", "other"])
   ...
   #BsmtFinType1
    ...
    #BsmtFinSF1
    BsmtFinSF1 = data[:,34].copy()
    BsmtFinSF1[BsmtFinSF1 == "NA"] = np.nan
    BsmtFinSF1 = BsmtFinSF1.astype(float)
    print(BsmtFinSF1[:10])
   ...
   #BsmtFinSF2
    BsmtFinSF2 = data[:,36].copy()
    BsmtFinSF2[BsmtFinSF2 == "NA"] = np.nan
    BsmtFinSF2 = BsmtFinSF2.astype(float)
    print(BsmtFinSF2[:10])

    
    #BsmtUnfSF
    BsmtUnfSF = data[:,37].copy()
    BsmtUnfSF[BsmtUnfSF == "NA"] = np.nan
    BsmtUnfSF = BsmtUnfSF.astype(float)
    print(BsmtUnfSF[:10])

    
    #TotalBsmtSF
    TotalBsmtSF = data[:,38].copy()
    TotalBsmtSF[TotalBsmtSF == "NA"] = np.nan
    TotalBsmtSF = TotalBsmtSF.astype(float)
    ...
    print(GrLivArea[:10])
        ...
        #BsmtFullBath
    BsmtFullBath = data[:,47].copy()
    BsmtFullBath[BsmtFullBath == "NA"] = np.nan
    BsmtFullBath = BsmtFullBath.astype(float)
    print(BsmtFullBath[:100])

        
    #BsmtHalfBath
    BsmtHalfBath = data[:,48].copy()
    BsmtHalfBath[BsmtHalfBath == "NA"] = np.nan
    BsmtHalfBath = BsmtHalfBath.astype(float)
    print(BsmtHalfBath[:100])

    #FullBath
    FullBath = data[:,49].copy()
    FullBath[FullBath == "NA"] = np.nan
    FullBath = FullBath.astype(float)
    print(FullBath[:100])

        
    #HalfBath
    HalfBath = data[:,50].copy()
    HalfBath[HalfBath == "NA"] = np.nan
    HalfBath = HalfBath.astype(float)
    print(HalfBath[:100])
        
    #Bedromm 
    Bedroom = data[:,51].copy()
    Bedroom[Bedroom == "NA"] = np.nan
    Bedroom = Bedroom.astype(float)
    print(Bedroom[:100])

        
    #Kitchen
    Kitchen = data[:,52].copy()
    Kitchen[Kitchen == "NA"] = np.nan
    Kitchen = Kitchen.astype(float)
    print(Kitchen[:100])
...
#Functional
    ...
    Functional[Functional == "NA"] = "other"

    functional_mapping = {"Typ": 0, "Min1": 1, "Min2": 2, "Mod": 3, 
                        "Maj1": 4, "Maj2": 5, "Sev": 6, "Sal": 7, "other": 8}
    ...
    #GarageCars
    GarageCars = data[:,61].copy()
    GarageCars[GarageCars == "NA"] = np.nan
    GarageCars = GarageCars.astype(float)
    print(GarageCars[:100])

    
   #GarageArea
    GarageArea = data[:,62].copy()
    GarageArea[GarageArea == "NA"] = np.nan
